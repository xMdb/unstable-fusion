...
def page_generate(username: str):
    ...
    for job in list(st.session_state["jobs"])[::-1]:
        future = job["future"]
        if not future.done():
            with st.status("⏳ Generating…", expanded=False) as status:
                st.write(f"Prompt: {job['prompt']}")
                st.write(f"Model: {job['model']} | Steps: {job['steps']} | Size: {job['w']}×{job['h']}")
                status.update(label="Running", state="running")
        else:
            if "images" not in job:
                # collect result once
                job["images"] = future.result()
                job["paths"] = []
                for img in job["images"]:
                    p = save_image(img, job["username"], job["prompt"])
                    job["paths"].append(p)

            st.success(f"✅ Job done (Prompt: {job['prompt']})")
            cols = st.columns(min(job["n_images"], 4))
            for idx, (img, p) in enumerate(zip(job["images"], job["paths"])):
                with cols[idx % len(cols)]:
                    st.image(img, caption=p.name, width="stretch")
                    st.download_button("Download", data=p.read_bytes(),
