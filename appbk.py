...
# Models accepted (strings accepted by your system; tried to match your request)
ALLOWED_MODELS = {
...
# ensure torch uses all available threads similar to your original script
...
# ---------------------------
# We'll maintain a pipeline pool per model (size = MAX_CONCURRENT). Each job checks out one pipeline instance.
pipelines_pools: Dict[str, Queue] = {}       # model_name -> Queue[StableDiffusionPipeline]
pipelines_lock = threading.Lock()
pipeline_init_lock = threading.Lock()        # protect concurrent pool initialization

# thread-safe counter for in-use pipelines (for reporting)
in_use_counter = 0
...
def ensure_pool_for_model(model_name: str):
    ...
    with pipeline_init_lock:
        ...
        # If any instance fails to load, we clean up and raise exception.
        created = []
        try:
            ...
            # (they are in-memory objects; Python GC will free them)
...
def generate_with_pipeline(job_prompt: str, width: int, height: int, model_name: str, out_path: str):
    ...
    try:
        ...
        # The pipeline instance is dedicated to this thread while checked out; this avoids thread-safety issues.
            ...
            def process_job(job_id_inner: int, model_name_inner: str):
                ...
                try:
                    ...
                    try:
                        ...
                        # Ensure pool exists (this can raise if model cannot be loaded)
                        ensure_pool_for_model(model_name_inner)

                        # Run generation (this checks out a pipeline instance from the pool)
                    ...
                    db2.close()
                    # decrement in-use and release semaphore slot
                    with in_use_counter_lock:
...
def startup_event():
    # On startup, reset any jobs that were processing back to queued (recover from crash)
    db = SessionLocal()
    try:
...
def download_image(image_id: int, current_user: User = Depends(get_user_from_token)):
    db = SessionLocal()
    try:
...
def health():
    with in_use_counter_lock:
        in_use = in_use_counter
    return {"ok": True, "concurrency_limit": MAX_CONCURRENT, "in_use": in_use}