...
def list_images(skip: int = 0, limit: int = 20, prompt_contains: Optional[str] = None, current_user: User = Depends(get_user_from_token)):
    db = SessionLocal()
    try:
        q = db.query(ImageModel, Job.model_name, Like.id.label('liked')).outerjoin(Job, ImageModel.job_id == Job.id).outerjoin(
        ...
        ) for i, model_name, liked in results]
    finally:
        db.close()
...
def get_image(image_id: int, current_user: User = Depends(get_user_from_token)):
    db = SessionLocal()
    try:
        result = db.query(ImageModel, Job.model_name, Like.id.label('liked')).outerjoin(Job, ImageModel.job_id == Job.id).outerjoin(
            Like, sa.and_(Like.image_id == ImageModel.id, Like.user_id == current_user.id)
